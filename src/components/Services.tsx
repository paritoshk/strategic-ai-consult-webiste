    ...
    <section id="services" className="section-padding relative">
      <div className="container mx-auto relative z-10">
        <div className="text-center mb-16">
          <span className="text-white text-sm uppercase tracking-wider font-serif">Our Services</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <div 
              key={index}
              className="glass-card rounded-none p-6 transform hover:-translate-y-1 transition-all duration-300 opacity-0 animate-fadeIn"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className="mb-4">{service.icon}</div>
              <h3 className="text-xl font-serif mb-3">{service.title}</h3>
              <p className="text-slate-light font-serif">{service.description}</p>
            </div>
          ))}
        </div>
        
        <div className="mt-20 glass-card rounded-none p-8 md:p-10">
          <div className="flex flex-col md:flex-row gap-10">
            <div className="md:w-1/2">
              <h3 className="text-2xl md:text-3xl font-serif">
                AI solutions that enable VCs to excel
              </h3>
              <p className="text-slate mt-4 font-serif">
              ...
              <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                {features.map((feature, index) => (
                  <div key={index} className="flex items-start gap-2">
                    ...
                    <span className="text-white font-serif">{feature}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="md:w-1/2 bg-black/50 rounded-none p-6 border border-white/10">
              <h4 className="text-xl font-serif mb-4">Our AI-Powered Approach</h4>
              <p className="text-slate-light mb-4 font-serif">
              ...
              <ul className="space-y-3">
                <li className="flex items-center gap-2">
                  ...
                  <span className="font-serif">Proprietary AI algorithms for investment analysis</span>
                </li>
                <li className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-white"></span>
                  <span className="font-serif">Natural language processing for market intelligence</span>
                </li>
                <li className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-white"></span>
                  <span className="font-serif">Machine learning models for trend prediction</span>
                </li>
                <li className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-white"></span>
                  <span className="font-serif">Custom dashboards for portfolio monitoring</span>
                </li>
              </ul>
            </div>
          </div>
      ...
      {/* Decorative elements */}
      <div className="absolute top-1/4 right-0 w-72 h-72 bg-white/10 rounded-full blur-[100px] opacity-30" />
    </section>
  );
};
